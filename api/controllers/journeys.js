...
exports.createJourneyFromPlaylist = async (req, res) => {
    try {
        ...
        // console.log(title,description); // Removed for production
        ...
        for (const video of videos) {
            // console.log(video.videoLink); // Removed for production
        ...
        logger.error(`Failed to create journey from YouTube playlist: ${error.message}`);
...
exports.forkJourney = async (req, res) => {
    try {
        ...
        logger.error(`Journey forking operation failed: ${error.message}`);
...
exports.getPublicJourneys = async (req, res) => {
    try {
      ...
      logger.error(`Failed to fetch public journeys: ${error.message}`);
      res.status(500).json({ error: 'Internal server error' });
    }
  };
