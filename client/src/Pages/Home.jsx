...
const Home = () => {
  ...
  const fetchData = async () => {
    ...
    if (journeys) {
      setData(journeys);
      logger.ui(`Loaded ${journeys.length} user journeys`);  
    }
  };
  ...
  return (
    <>
      <section className=" min-h-[90vh] bg-gray-50 dark:bg-gray-900 p-3 sm:p-5 antialiased ">
        <h1 className="py-10 mx-auto max-w-screen-xl text-gray-900 dark:text-white font-bold text-2xl ">
          Your Journeys
        </h1>
                  ...
                  {data &&
                    data.map((d, i) => (
                      <tr className="border-b dark:border-gray-700" key={d.id}>
                        ...
                        <Link className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 cursor-pointer hover:underline font-medium" to={`/journey/${d.id}`}>
                          {d.title ? d.title : "Untitled"}
                        </Link>
                        </td>
                        <td className="px-4 py-3 max-w-[12rem] truncate text-gray-700 dark:text-gray-300">
                          {d.description
                            ? d.description
